...
if recordData:
    ...
    expName = 'M3LetterSizeEccentricity'
...
headers = ['Letter', 'Direction', 'Eccentricity', 'Reaction Time (s)', 'Correct?', 'size']
...
def instructions():
    genDisplay({'text': 'Press "V" when you see the "B" displayed',\
        'xPos': 0, 'yPos': centerY+4, 'heightCm': 1, 'color': 'white'}).draw()
    genDisplay({'text': 'and "B" when you see the "A" displayed',\
        'xPos': 0, 'yPos': centerY+2, 'heightCm': 1, 'color': 'white'}).draw()
    genDisplay({'text': 'and "N" when you see the "M" displayed',\
...
def interpretPair(pair):
    ...
    return {'angle': angle, 'size': size, 'dir': direction, 'letter': letter}
...
for pair in pairs:
    ...
    time.sleep(.1)
    win.flip()
    lettersize = trialInfo['size']
    stimheight = displaceCalc(lettersize)*heightMult
    ...
    if buffer > 0:
        if checkcorrect(key[0], trialInfo['letter']):
            output = (trialInfo['letter'], trialInfo['dir'], trialInfo['angle'], reactionTime, '1', lettersize)
            csvOutput(output, fileName)
        else:
            mistakedict[mistakes] = trialInfo
            mistakes += 1
            output = (trialInfo['letter'], trialInfo['dir'], trialInfo['angle'], reactionTime, '0', lettersize)
            csvOutput(output, fileName)
    else:
        mistakedict[mistakes] = trialInfo
        mistakes += 1
