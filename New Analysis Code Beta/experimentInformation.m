function [folderName, axes, refDist, dataFolder, ...
    protOrder, protNames, colors, linestyles] = experimentInformation()
[file, path] = uigetfile("*.csv;*.xlsx", "Choose Analysis Template");
template = readtable(fullfile(path, file));

format shortg;
userInput = inputdlg('All analysis files will be organized into a folder with this name:', ...
    'Analysis Folder Name', [2 60], template{1,1});
...
axes = inputdlg({'Enter x axis label', 'Enter y axis label'}, ...
    'Axes Names', [2 35], template{[2,1],3});

refDist = inputdlg('Enter expected location of kink:', ...
    'Kink Location', [2 35], template{1,6} + "");
...
protOrder = str2double(inputdlg({message, protNames{2:end}}, "Plotting Order", [1 35], order + ""));
...
for i = 1:max(protOrder)
    for j = 1:length(protOrder)
        if i == protOrder(j)
            ...
            newDataDir(i,1) = dataDir(j);
        end
    end
end
dataDir = [dataDir(1:2); newDataDir];
protNames = inputdlg(newProtNames, "Rename Protocols", [1 35], newTemplate{:,7});
...
colors = inputdlg([message + protNames(1); protNames(2:end)], ...
    'Choose plotting colors', [1,35], template{:,9});

message = 'Choose from these linestyles: ''-'' (solid) or ''--'' (dashed)';
linestyles = inputdlg([message + protNames(1); protNames(2:end)], ...
    'Choose line drawing styles', [1,35], template{:,10}); 